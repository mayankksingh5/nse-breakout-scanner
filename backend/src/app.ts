...
// In-memory global arrays to simulate real live structural tables data frame
let globalSignals: any[] = [];

// GET /api/signals - Pure Array Return Block for Dashboard Data Integrity
app.get('/api/signals', async (req: Request, res: Response) => {
  try {
    // If memory is empty, send a mock analytical array instantly instead of returning empty state
...
// GET /api/engine/seed - Easy Direct Data Injections
app.get('/api/engine/seed', async (req: Request, res: Response) => {
  try {
    globalSignals = [
      { id: '1', symbol: 'RELIANCE', company_name: 'Reliance Industries Ltd', current_price: 2496.40, resistance_price: 2470.00, distance_pct: 1.06, price_change_pct: 3.20, current_volume: 8500000, avg_volume_30d: 4000000, volume_ratio: 2.12, prev_resistance_volume: 5000000, breakout_score: 94, signal_type: 'STRONG_BUY' },
      { id: '2', symbol: 'TCS', company_name: 'Tata Consultancy Services Ltd', current_price: 3950.00, resistance_price: 3920.00, distance_pct: 0.76, price_change_pct: 2.45, current_volume: 3200000, avg_volume_30d: 1500000, volume_ratio: 2.13, prev_resistance_volume: 2000000, breakout_score: 88, signal_type: 'STRONG_BUY' },
    ...
    res.json({ status: "Database bypassed successfully. 4 Core bluechip breakout assets processed directly in local-memory frames!" });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
app.listen(PORT, () => console.log(`Scanner server active on port ${PORT}`));